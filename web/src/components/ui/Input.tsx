...
export function Input({ label, error, helper, className = '', ...props }: InputProps) {
  return (
    <label className="block space-y-1.5">
      ...
      <input className={`focus-ring w-full rounded-xl border border-black/15 bg-white px-3 py-2.5 text-ink transition-colors placeholder:text-zinc-400 dark:border-white/10 dark:bg-white/10 dark:text-white dark:placeholder:text-white/35 ${className}`} {...props} />
