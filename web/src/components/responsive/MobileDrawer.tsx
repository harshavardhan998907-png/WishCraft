    ...
    <div className={`fixed inset-0 z-50 lg:hidden ${open ? '' : 'pointer-events-none'}`} aria-hidden={!open}>
      ...
      >
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button type="button" className="focus-ring min-h-11 rounded-md px-3 text-sm font-bold text-ink hover:bg-black/5 dark:text-white dark:hover:bg-white/10" onClick={onClose}>
            Close
          </button>
        </div>
        {children}
